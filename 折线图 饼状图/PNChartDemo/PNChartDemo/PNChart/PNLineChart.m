...
#define kXLinePointFit 30 // 每个点同步左右移动
#define kYLinePointFit 20 // 每个点同步上下移动
#define kYAcxicLabelsYFit 20 // Y轴Label同步上下移动
#define kYAcxicLabelsXFit 5 // Y轴Label同步左右移动
#define kXLinePointCoefficient 0.9 // 每个点左右同步左右缩放比
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height
{
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            NSInteger y = (NSInteger)(_chartCavanHeight - index * yStepHeight) + kYAcxicLabelsYFit;
            
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0 + kYAcxicLabelsXFit, y, (NSInteger)_chartMargin, (NSInteger)_yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width
{
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = 2 * _chartMargin +  (index * _xLabelWidth) - (_xLabelWidth / 2) - _xLabelWidth /2.5;
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            CGFloat offSetX = (_chartCavanWidth) / (chartData.itemCount) * kXLinePointCoefficient;
...
//                [gradePathArray addObject:textLayer];
                ...
                if ( i != 0 ) {
                    [progressline addLineToPoint:CGPointMake(x - kXLinePointFit, y + kYLinePointFit)];
                    [lineStartEndPointsArray addObject:[NSValue valueWithCGPoint:CGPointMake(x - kXLinePointFit, y + kYLinePointFit)]];
                }
                
                [progressline moveToPoint:CGPointMake(x - kXLinePointFit, y + kYLinePointFit)];
                if(i != chartData.itemCount - 1){
                    [lineStartEndPointsArray addObject:[NSValue valueWithCGPoint:CGPointMake(x - kXLinePointFit, y + kYLinePointFit)]];
                }
            }
            
            [linePointsArray addObject:[NSValue valueWithCGPoint:CGPointMake(x - kXLinePointFit, y + kYLinePointFit)]];
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleNone) {
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    if (i == chartData.itemCount - 1) {
                        ...
                        view.image = [UIImage imageNamed:@"circle"];
                        view.backgroundColor = [UIColor blueColor];
                        view.layer.masksToBounds = YES;
                        view.layer.cornerRadius = 5;
                        UIBezierPath *bezierPath = _chartPath[lineIndex];
                        view.center = bezierPath.currentPoint;
                        [self addSubview:view];
                        
                        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 40, 15)];
                        imageView.image = [UIImage imageNamed:@"labelBG_red"];
                        imageView.center = CGPointMake(bezierPath.currentPoint.x + 25, bezierPath.currentPoint.y);
                        [self addSubview:imageView];
                        
                        
                        descriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 38, 15)];
                        descriptionLabel.center = CGPointMake(bezierPath.currentPoint.x + 25, bezierPath.currentPoint.y);
                        descriptionLabel.textAlignment = NSTextAlignmentCenter;
                        descriptionLabel.font = [UIFont boldSystemFontOfSize:10.0f];
                        descriptionLabel.textColor = [UIColor whiteColor];
                        descriptionLabel.text = self.lastPointValue;
...
#pragma mark - Set Chart Data 渐变色
    ...
    if (data != _chartData) {
        ...
        for (PNLineChartData *chartData in data) {
            ...
            if (self.shadowColor) {
                ...
                NSInteger count = self.shadowColorArray.count;
                if (count > 1) {
                    for (int i = 0; i < count - 1; i++) {
                        CAGradientLayer *gradientLayer1 = [CAGradientLayer layer];
                        gradientLayer1.frame = CGRectMake(self.bounds.size.width / (count - 1) * i, 0, self.bounds.size.width / (count - 1), self.bounds.size.height);
                        [gradientLayer1 setColors:[NSArray arrayWithObjects:(id)[_shadowColorArray[i] CGColor],(id)[_shadowColorArray[i + 1] CGColor], nil]];
