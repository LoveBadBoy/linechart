...
- (void)recompute {
    ...
    self.innerCircleRadius = CGRectGetWidth(self.bounds) / 4;
}
    ...
    for (int i = 0; i < _items.count; i++) {
        if (self.showImageView) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                UIImageView *descriptionImageView = [self descriptionsLabelForItemAtIndex:i];
                [_contentView addSubview:descriptionImageView];
                [_descriptionLabels addObject:descriptionImageView];
                descriptionImageView.alpha = 0.0;
                [UIView animateWithDuration:1.0f animations:^{
                    descriptionImageView.alpha = 1.0;
                }];
            });
...
#pragma mark -- 添加图片的方法
- (UIImageView *)descriptionsLabelForItemAtIndex:(NSUInteger)index
{
    ...
    UIImageView *descriptionImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 100, 80)];
    UIImage *titleImage = currentDataItem.titleImage;
    
    descriptionImageView.image = titleImage;
    
    //If value is less than cutoff, show no imageView
    ...
    descriptionImageView.frame = CGRectMake(descriptionImageView.frame.origin.x, descriptionImageView.frame.origin.y,
                                            30, 30);
    descriptionImageView.center          = center;
    descriptionImageView.alpha           = 0;
    descriptionImageView.backgroundColor = [UIColor clearColor];
...
- (void)didTouchAt:(CGPoint)touchLocation
{
    ...
    // 添加旋转效果
    if (self.transformAnimation) {
        CGFloat forwardValue = [_endPercentages[index] floatValue];
        CGFloat towardValue = 0.0;
        if (index - 1 >= 0) {
            towardValue = [_endPercentages[index - 1] floatValue];
        }
        CGFloat keyPercent = (forwardValue - towardValue) / 2 + towardValue;
        
        [UIView animateWithDuration:0.5 animations:^{
            self.transform = CGAffineTransformMakeRotation(M_PI_4 / 45 * (180 - 360 * keyPercent));
            for (int i = 0; i < self.descriptionLabels.count; i++) {
                UIImageView *imageView = (UIImageView *)self.descriptionLabels[i];
                imageView.transform = CGAffineTransformMakeRotation(M_PI_4 / 45 * (360 - (180 - 360 * keyPercent)));
            }
        }];
        if (self.touchShake) {
            CAKeyframeAnimation *keyframeAnimation = [CAKeyframeAnimation animation];
//            keyframeAnimation.keyPath = @"position.x";
//            keyframeAnimation.values = @[@0, @10, @0, @(-10), @0];
            keyframeAnimation.keyPath = @"bounds";
            NSValue *value1=[NSValue valueWithCGRect:CGRectMake(0, 0, 15, 15)];
            NSValue *value2=[NSValue valueWithCGRect:CGRectMake(0, 0, 20, 20)];
            NSValue *value3=[NSValue valueWithCGRect:CGRectMake(0, 0, 15, 15)];
            NSValue *value4=[NSValue valueWithCGRect:CGRectMake(0, 0, 10, 10)];
            NSValue *value5=[NSValue valueWithCGRect:CGRectMake(0, 0, 15, 15)];
            ...
            UIImageView *imageView = (UIImageView *)self.descriptionLabels[index];
            [imageView.layer addAnimation:keyframeAnimation forKey:@"keyframe"];
